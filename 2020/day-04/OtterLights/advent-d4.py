...
for passport in input:
    ...
    if not("byr" in info and int(info["byr"]) >= 1920 and int(info["byr"]) <= 2002 and "iyr" in info and int(info["iyr"]) >= 2010 and int(info["iyr"]) <= 2020 and "eyr" in info and int(info["eyr"]) >= 2020 and int(info["eyr"]) <= 2030):
        valid = False
     
        
    if "hgt" in info and info["hgt"][:len(info["hgt"])-2].isdigit():
        hgtNum = int(info["hgt"][:len(info["hgt"])-2])
        if (info["hgt"][len(info["hgt"])-2:] == "cm" and hgtNum >= 150 and hgtNum <= 193) or (info["hgt"][len(info["hgt"])-2:] == "in" and hgtNum >= 59 and hgtNum <= 76):
            print("tst")
        else:
            valid = False
    else:
    ...
    if "hcl" in info and info["hcl"][0] == '#':
        for letter in info["hcl"][1:]:
            if letter != "0" and letter != "1" and letter != "2" and letter != "3" and letter != "4" and letter != "5" and letter != "6" and letter != "7" and letter != "8" and letter != "9" and letter != "a" and letter != "b" and letter != "c" and letter != "d" and letter != "e" and letter != "f":
                #invalid
                valid = False
    else:
        valid = False
        
    if not("ecl" in info and (info["ecl"] == "amb" or info["ecl"] == "blu" or info["ecl"] == "brn" or info["ecl"] == "gry" or info["ecl"] == "grn" or info["ecl"] == "hzl" or info["ecl"] == "oth")):
        valid = False
    if not("pid" in info and info["pid"].isdigit() and len(info["pid"]) == 9):
        valid = False
