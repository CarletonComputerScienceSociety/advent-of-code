input_string = '''16-18 h: hhhhhhhhhhhhhhhhhh
...
for p in passwords:
    ...
    policy = p[:p.index(':')]
    minimum = int(policy[:policy.index('-')])
    maximum = int(policy[policy.index('-')+1:policy.index(' ')])
    ...
    if letter_to_check in letters:
        if minimum <= letters[letter_to_check] <= maximum:
            valid += 1
